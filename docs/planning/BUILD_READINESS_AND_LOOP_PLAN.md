...
| ADR-09 | **Multi-product tenancy & product hierarchy** (David's constraint) | Self-referential `Product` tree (peers now, sub-offerings later) + `product_id` scoping from commit 1. Full record in `docs/decisions/DECISION_PACK.md`; implemented by spec-patch B0. |
...
- **Needs human decision (not verification):** the 9 ADRs are business/infra choices only David can make; the content seeds (ADR-08) are facts only David can author.
